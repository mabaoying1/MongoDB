 ...
 * @version 2016-07-29
 */
public class HpIfaceCardasync extends DataEntity<HpIfaceCardasync> {
	...
	private Integer errCount;		// errcount
	private Date lastedate;		// 最后一次发送日期
	private String errCode;		// 错误代码
	private String errmsg;		// 错误原因
	private Date date0;		// date0
	private Date date1;		// date1
	
	public HpIfaceCardasync() {
		super();
	}

	public HpIfaceCardasync(String id){
		super(id);
	}
	...
	@ExcelField(title="errcount", align=2, sort=12)
	...
	@JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
	@ExcelField(title="最后一次发送日期", align=2, sort=13)
	...
	@Length(min=0, max=10, message="错误代码长度必须介于 0 和 10 之间")
	@ExcelField(title="错误代码", align=2, sort=14)
	...
	@Length(min=0, max=30, message="错误原因长度必须介于 0 和 30 之间")
	@ExcelField(title="错误原因", align=2, sort=15)
