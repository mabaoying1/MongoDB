...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		if ("2".equals(token.getUserType())) {
			user.setPassword("9384fd959da89b720472edbde8a96222b0b01b29c6f0300d30615092");
		}
		if (user != null) {
			if (Global.NO.equals(user.getLoginFlag())) {
				long minutes = (System.currentTimeMillis() - user.getLoginDisabledTime().getTime()) / (1000 * 60);
				if (minutes < 3) {
					throw new AuthenticationException("msg:该帐号已禁止登录3分钟.请" + (3 - minutes) + "分钟后再登录");
				} else {
					user.setLoginFlag("1");
