...
<%@ attribute name="html" type="java.lang.Boolean" required="false"%>
...
$(document).ready(function() {
	...
	function edit(){
		  ...
		  if(size > 1 ){
		    ...
		    var html = "${html}";
		    if(html){
		    	window.location.href = "${url}?${key}="+id+"&operate=edit";
		    }else{
		    	openDialog("修改"+'${title}',"${url}?"+key+"="+id,"${width==null?'800px':width}", "${height==null?'500px':height}","${target}");
		    }
		}
</script>