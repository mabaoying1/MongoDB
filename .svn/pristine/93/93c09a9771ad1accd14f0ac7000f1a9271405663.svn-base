...
<%@ attribute name="key" type="java.lang.String" required="false"%>
<button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="audit()" title="审核"><i class="fa fa-file-text-o"></i> ${label==null?'审核':label}</button>
                        </button>
...
$(document).ready(function() {
	...
	function audit(){
		  ...
		  if(size > 1 ){
		    ...
		    //openAuditDialog("审核"+'${title}',"${url}?${key}="+id,"${width==null?'800px':width}", "${height==null?'500px':height}","${target}");
		}
</script>