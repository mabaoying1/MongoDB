...
import com.healthpay.common.security.Digests;
...
import com.healthpay.modules.sys.utils.LogUtils;
...
public class SystemService extends BaseService implements InitializingBean {
	...
	public List<Role> findRole(Role role) {
		User user = role.getCurrentUser();

		role.getSqlMap().put("dsf", BaseService.dataScopeFilter(user, "oa", ""));
	...
	public Page<User> findUser(Page<User> page, User user) {
		...
		// 执行分页查询
		List<User> list = userDao.findList(user).stream().peek(u -> {
			if (u.getPhone() != null) {
				u.setPhone(u.getPhone().replaceAll("(\\d{3})\\d{4}(\\d{4})", "$1****$2"));
			}
			if (u.getMobile() != null) {
				u.setMobile(u.getMobile().replaceAll("(\\d{3})\\d{4}(\\d{4})", "$1****$2"));
			}
		...
		if (StringUtils.isBlank(user.getId())) {
			...
			if ((oldUser.getArea() != null) && (oldUser.getArea().getId() != null)) {
				CacheUtils.remove(UserUtils.USER_CACHE,
						UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + oldUser.getArea().getId());
			}
