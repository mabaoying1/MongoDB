 ...
 */
package com.healthpay.common.service;
...
import com.healthpay.common.persistence.BaseEntity;
...
public abstract class BaseService {
			...
			for (Role r : user.getRoleList()) {
				for (String oa : StringUtils.split(officeAlias, ",")) {
					if (!dataScope.contains(r.getDataScope()) && StringUtils.isNotBlank(oa)) {
						...
						// }
						// else if
						// (Role.DATA_SCOPE_COMPANY.equals(r.getDataScope())){
						// sqlString.append(" OR " + oa + ".id = '" +
						// user.getCompany().getId() + "'");
						// // 包括本公司下的部门 （type=1:公司；type=2：部门）
						// sqlString.append(" OR (" + oa + ".parent_id = '" +
						// user.getCompany().getId() + "' AND " + oa + ".type =
						// '2')");
						// }
						else if (Role.DATA_SCOPE_AREA_AND_CHILD.equals(r.getDataScope())) {
							sqlString.append(" OR " + oa + ".id = '" + user.getArea().getId() + "'");
							sqlString.append(" OR " + oa + ".parent_ids LIKE '" + user.getArea().getParentIds()
									+ user.getArea().getId() + ",%'");
						} else if (Role.DATA_SCOPE_AREA.equals(r.getDataScope())) {
							sqlString.append(" OR " + oa + ".id = '" + user.getArea().getId() + "'");
	...
	public static void dataScopeFilter(BaseEntity<?> entity, String sqlMapKey, String officeWheres, String userWheres) {
		...
		for (String where : StringUtils.split(officeWheres, ",")) {
			if (Role.DATA_SCOPE_AREA_AND_CHILD.equals(dataScopeString)) {
				// 包括本公司下的部门 （type=1:公司；type=2：部门）
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_AREA");
				sqlString.append(" WHERE (id = '" + user.getArea().getId() + "'");
				sqlString.append(" OR parent_ids LIKE '" + user.getArea().getParentIds() + user.getArea().getId()
						+ ",%')");
				sqlString.append(" AND " + where + ")");
			} else if (Role.DATA_SCOPE_AREA.equals(dataScopeString)) {
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_AREA");
				sqlString.append(" WHERE id = '" + user.getArea().getId() + "'");
				sqlString.append(" AND " + where + ")");
