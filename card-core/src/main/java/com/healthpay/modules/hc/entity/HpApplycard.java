 ...
 * @version 2016-05-25
 */
public class HpApplycard extends DataEntity<HpApplycard> {
	
	private static final long serialVersionUID = 1L;
	private String  pkid;		// 主键uuid
	private String identityId;		// 身份证号码 （主键，程序控制18位） //证件类型+证件号码
	...
	@Length(min=0, max=11, message="本人手机长度必须介于 0 和 11 之间")
	@ExcelField(title="本人手机", align=2, sort=42)
	@Pattern(regexp = "^((13[0-9])|(15[^4,\\D])|(18[0-9])|(17[0-9]))\\d{8}$" ,message="手机格式不正确")
	...
	@Length(min=0, max=30, message="申请时为空，审核通过分配长度必须介于 0 和 30 之间")
	...
	@ExcelField(title="是否开通健康钱包  0 - 否  1 - 是", align=2, sort=73)
	...
	@ExcelField(title="支付限额 0 or null  表示没有限额", align=2, sort=74)
	...
	@ExcelField(title="支付密码  开通支付必须填密码", align=2, sort=75)
	...
	@ExcelField(title="支付系统类型（1 银行 2 银联 3 APPLEPAY...建立数据字典）", align=2, sort=81)
	...
	@ExcelField(title="支付类型为银行时 ，记录银行", align=2, sort=85)
	...
	@ExcelField(title="bankname", align=2, sort=86)
	...
	@ExcelField(title="账号类型 0 借记卡 1 信用卡", align=2, sort=87)
	...
	@ExcelField(title="身份证正反面上传标识：0-未上传 1-已上传", align=2, sort=90)
	...
	public boolean getIsNewRecord(){
		...
		return isNewRecord || getPkid()==null;
	}
