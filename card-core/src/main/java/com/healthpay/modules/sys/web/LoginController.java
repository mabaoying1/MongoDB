...
import com.healthpay.modules.iim.utils.DateUtil;
...
//		}
		...
		Integer loginFailNum =  getlLoginFailNum(username);
		...
		if(loginFailNum>=3){
			...
			model.addAttribute("isValidateCodeLogin",false);
		}
...
//		}
		...
		if (!UnauthorizedException.class.getName().equals(exception)){
			...
			if(user!=null){
				int loginFailNum = user.getLoginFailNum();
				loginFailNum++;
				if(loginFailNum<=4){
					...
					user.setLoginFailNum(loginFailNum);
					user.setLoginFlag("0");
					user.setLoginDisabledTime(new Date());
				}
	 ...
	 * @parma username
	 */
	private Integer getlLoginFailNum(String username){
		Map<String, Integer> loginFailMap = (Map<String, Integer>)CacheUtils.get("loginFailMap");
		Integer loginFailNum = 0;
		if(loginFailMap!=null){
			//loginFailNum = loginFailMap.get(UserUtils.getUser().getLoginName());
			loginFailNum = loginFailMap.get(username);
		}
		return loginFailNum;
	}
	/**
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		...
		user.setLoginDisabledTime(null);
		userService.update(user);
		if (logger.isDebugEnabled()){
...
//		request.setAttribute("mailPage", mailPage);
	...
	public String home(HttpServletRequest request, HttpServletResponse response, Model model) throws IOException {
		//消息提醒
        if(SecurityUtils.getSubject().isPermitted("hc:hpApplycard:list")){
            HpApplycard entity = new HpApplycard();
            int count = hpApplycardService.getWaitAuditCount(entity);
            model.addAttribute("count",count);
        }
		HpApplycard entity = new HpApplycard();
		Page<HpApplycard> page = hpApplycardService.findPage(new Page<HpApplycard>(request, response,4), entity);
		model.addAttribute("page", page);
		User currentUser = UserUtils.getUser();
		model.addAttribute("user", currentUser);
        //查询本年开卡情况
        HpCardholder hpCardholder = new HpCardholder();
		hpCardholder.setYear(DateUtils.getYear());
        Map<String,Integer> map = residentService.findChartData(hpCardholder);
        model.addAttribute("data",map);
		model.addAttribute("year",hpCardholder.getYear());
