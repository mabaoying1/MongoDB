...
@Controller
@RequestMapping(value = "${adminPath}/analysis/resistat")
public class ResiStatController extends BaseController {
    @Autowired
    private ResidentService residentService;

    @RequiresPermissions("analysis:resistat:list")
    @RequestMapping(value = { "list", "" })
    public String list(HpCardholder hpCardholder, HttpServletRequest request, HttpServletResponse response,
                       Model model) {
        Page<HpCardholder> page = residentService.findPag(new Page<HpCardholder>(request, response), hpCardholder);

        model.addAttribute("page", page);

        return "modules/analysis/resiStatList";
    }
}
